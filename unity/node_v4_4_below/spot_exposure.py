...
class CompositorNodeSpotExposure(bpy.types.CompositorNodeCustomGroup, Node, Mask_Node):
	...
	def getNodetree(self, context):
		...
		#Socket Exposure
		exposure_socket = node_tree.interface.new_socket(name = "Exposure", in_out='INPUT', socket_type = 'NodeSocketFloat')
		exposure_socket.default_value = 0.0
		exposure_socket.min_value = -10.0
		exposure_socket.max_value = 10.0
		exposure_socket.subtype = 'NONE'
		...
		node_tree.links.new(group_input.outputs[2], blur.inputs[1])
		#blur.Image -> mix.Fac
		...
		#exposure.Image -> mix.Image
		node_tree.links.new(exposure.outputs[0], mix.inputs[1])
